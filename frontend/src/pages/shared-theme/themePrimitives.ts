import { createTheme, alpha, Shadows, ThemeOptions } from '@mui/material/styles';

// Extensão do Paper
declare module '@mui/material/Paper' {
  interface PaperPropsVariantOverrides {
    highlighted: true;
  }
}

// Extensão da Palette
declare module '@mui/material/styles' {
  interface ColorRange {
    50: string;
    100: string;
    200: string;
    300: string;
    400: string;
    500: string;
    600: string;
    700: string;
    800: string;
    900: string;
  }

  interface PaletteColor extends ColorRange {}

  interface Palette {
    baseShadow: string;
  }

  interface PaletteOptions {
    baseShadow?: string; // permite passar baseShadow sem erro
  }
}

// Tema padrão
const defaultTheme = createTheme();
const customShadows: Shadows = [...defaultTheme.shadows];
const baseShadowValue = customShadows[1];

// Cores
export const brand = {
  50: 'hsl(210, 100%, 95%)',
  100: 'hsl(210, 100%, 92%)',
  200: 'hsl(210, 100%, 80%)',
  300: 'hsl(210, 100%, 65%)',
  400: 'hsl(210, 98%, 48%)',
  500: 'hsl(210, 98%, 42%)',
  600: 'hsl(210, 98%, 55%)',
  700: 'hsl(210, 100%, 35%)',
  800: 'hsl(210, 100%, 16%)',
  900: 'hsl(210, 100%, 21%)',
};

export const gray = {
  50: 'hsl(220, 35%, 97%)',
  100: 'hsl(220, 30%, 94%)',
  200: 'hsl(220, 20%, 88%)',
  300: 'hsl(220, 20%, 80%)',
  400: 'hsl(220, 20%, 65%)',
  500: 'hsl(220, 20%, 42%)',
  600: 'hsl(220, 20%, 35%)',
  700: 'hsl(220, 20%, 25%)',
  800: 'hsl(220, 30%, 6%)',
  900: 'hsl(220, 35%, 3%)',
};

export const green = {
  50: 'hsl(120, 80%, 98%)',
  100: 'hsl(120, 75%, 94%)',
  200: 'hsl(120, 75%, 87%)',
  300: 'hsl(120, 61%, 77%)',
  400: 'hsl(120, 44%, 53%)',
  500: 'hsl(120, 59%, 30%)',
  600: 'hsl(120, 70%, 25%)',
  700: 'hsl(120, 75%, 16%)',
  800: 'hsl(120, 84%, 10%)',
  900: 'hsl(120, 87%, 6%)',
};

export const orange = {
  50: 'hsl(45, 100%, 97%)',
  100: 'hsl(45, 92%, 90%)',
  200: 'hsl(45, 94%, 80%)',
  300: 'hsl(45, 90%, 65%)',
  400: 'hsl(45, 90%, 40%)',
  500: 'hsl(45, 90%, 35%)',
  600: 'hsl(45, 91%, 25%)',
  700: 'hsl(45, 94%, 20%)',
  800: 'hsl(45, 95%, 16%)',
  900: 'hsl(45, 93%, 12%)',
};

export const red = {
  50: 'hsl(0, 100%, 97%)',
  100: 'hsl(0, 92%, 90%)',
  200: 'hsl(0, 94%, 80%)',
  300: 'hsl(0, 90%, 65%)',
  400: 'hsl(0, 90%, 40%)',
  500: 'hsl(0, 90%, 30%)',
  600: 'hsl(0, 91%, 25%)',
  700: 'hsl(0, 94%, 18%)',
  800: 'hsl(0, 95%, 12%)',
  900: 'hsl(0, 93%, 6%)',
};

// Função para gerar tokens do tema
export const getDesignTokens = (mode: 'light' | 'dark'): ThemeOptions => {
  customShadows[1] =
    mode === 'dark'
      ? 'hsla(220, 30%, 5%, 0.7) 0px 4px 16px 0px, hsla(220, 25%, 10%, 0.8) 0px 8px 16px -5px'
      : 'hsla(220, 30%, 5%, 0.07) 0px 4px 16px 0px, hsla(220, 25%, 10%, 0.07) 0px 8px 16px -5px';

  return {
    palette: {
      mode,
      primary: { main: brand[400] },
      grey: { ...gray },
      baseShadow: customShadows[1],
      background: {
        default: mode === 'dark' ? gray[900] : gray[50],
        paper: mode === 'dark' ? gray[800] : gray[100],
      },
      text: {
        primary: mode === 'dark' ? gray[50] : gray[900],
        secondary: mode === 'dark' ? gray[300] : gray[600],
      },
    },
    shadows: customShadows,
    typography: {
      fontFamily: 'Inter, sans-serif',
      h1: { fontSize: '3rem', fontWeight: 600, lineHeight: 1.2 },
      h2: { fontSize: '2.25rem', fontWeight: 600, lineHeight: 1.2 },
      h3: { fontSize: '1.875rem', lineHeight: 1.2 },
      h4: { fontSize: '1.5rem', fontWeight: 600, lineHeight: 1.5 },
      h5: { fontSize: '1.25rem', fontWeight: 600 },
      h6: { fontSize: '1.125rem', fontWeight: 600 },
      body1: { fontSize: '1rem' },
      body2: { fontSize: '1rem', fontWeight: 400 },
      subtitle1: { fontSize: '1.125rem' },
      subtitle2: { fontSize: '0.875rem', fontWeight: 500 },
      caption: { fontSize: '0.75rem', fontWeight: 400 },
    },
    shape: { borderRadius: 8 },
  };
};
