...
const SMART_DEVICE_CATEGORIES = ['Casa Inteligente', 'Wearables', 'Segurança'];
...
export default function SmartDevicesProducts(props: SmartDevicesProps) {
  ...
  const [products, setProducts] = React.useState<SmartDeviceProduct[]>([]);
    ...
    const fetchProducts = async () => {
      try {
        ...
        const filteredAndMapped = apiProducts
          .filter((product) => SMART_DEVICE_CATEGORIES.includes(product.category))
          .map((product) => ({
            id: product.id,
            name: product.name,
            price: product.price,
  ...
  const handleAddToCart = (product: SmartDeviceProduct) => {
